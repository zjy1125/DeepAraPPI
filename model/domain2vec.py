    ...
    with open(embedding_file) as f:
        for line in f:
            if line.startswith('>'):
                ...
                vec = np.array([vec])
                all_pro_vec[pro] = vec   
all_pro_domain2vec = get_embeddings('embeddings/ara_domain2vec_embeddings.txt')
...
for train, test in train_test:
    ...
    merge_model.save('domain2vec_model'+str(k)+'.h5')
    pred = merge_model.predict([X_train_array_1[test],X_train_array_2[test]])
    np.savetxt('domain2vec_val'+str(k)+'.txt',pred,fmt='%s',delimiter='\t')
    test_pred = merge_model.predict([X_test_array_1, X_test_array_2])
    np.savetxt('domain2vec_test'+str(k)+'.txt',test_pred,fmt='%s',delimiter='\t')
    k+=1
